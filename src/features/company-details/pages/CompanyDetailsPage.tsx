...
export const CompanyDetailsPage = () => {
  ...
  const [filters, setFilters] = useState<Partial<Record<keyof CompanyRow, string>>>({});
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const response = await companyDetailsService.getCompanies(page - 1, pageSize);
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-semibold text-white">Detalles de Compañía</h1>
          <p className="text-padsa-text-secondary mt-1">Administración de datos generales de compañías.</p>
        </div>
